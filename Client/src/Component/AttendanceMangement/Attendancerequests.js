    ...
    <div className="admin-attendance-reqp-container">
      ...
      <div className="employee-stats">
        ...
        <div className="stat-box active-stats">
          ...
          <p>
            <CountUp start={0} end="1" duration={2} separator="," />
          </p>
        </div>
      ...
      <div>
        <div className="filter-section">
          ...
          <button
            className="admin-attendance-reqp-action-button"
            onClick={handlePopup}
          >
            Action
          </button>
        ...
        <table className="admin-attendance-reqp-table">
          <thead>
            <tr>
              ...
              {["EmployeeID", "Name", "Date", "Reason", "Status"].map(
                (header) => (
                  <th
                    key={header}
          ...
          <tbody>
            {sortedDeals.map((deal) => (
              <tr key={deal.id}>
                ...
                <td>{deal.reason}</td>
                <td
                  className={`admin-attendance-reqp-status ${deal.status.toLowerCase()}`}
                >
                  {deal.status}
                </td>
              </tr>
      ...
      {popup.visible && (
        <div className="admin-attendance-reqp-popup-overlay">
          <div className="admin-attendance-reqp-popup">
            <p className="admin-attendance-reqp-popup-content">
              Accept or Reject Selected Requests?
            </p>
            <button
              className="admin-attendance-reqp-accept"
              onClick={closePopup}
            >
              Accept
            </button>
            <button
              className="admin-attendance-reqp-reject"
              onClick={closePopup}
            >
              Reject
            </button>
